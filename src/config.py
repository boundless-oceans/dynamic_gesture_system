...
# 用帧间差异判断"有没有动作"，静止时跳过推理，一有动作立刻恢复——不增加延迟。
#
# 默认关闭，原因：省下的算力**强依赖于现场的光线与摄像头**。
# 实测（本机摄像头、80x60 + 5x5 模糊 + 阈值 0.004）：静止画面的误报率约 3%，
# 门控真正闲下来的时间占 15%~40%——有收益，但远不是我最初估的"几乎全部"。
# 摄像头噪声大的话，误报会把门控一直撑开，一点也省不下来。
#
# 建议：到现场后再决定是否打开。打开前先用 tests/manual/smoke_inference.py
# 看识别是否正常，并留意终端里"画面静止，暂停推理"的日志是否正常出现/恢复。
# 灵敏度可用设置页的"动静门控灵敏度"现场微调。
MOTION_GATE_ENABLED = False
# 门控开启时，静止期间推理线程的轮询间隔（毫秒）。越小恢复越及时，越大越省。
MOTION_IDLE_POLL_MS = 50
# 最后一次动作之后仍认为"有动作"的宽限期（毫秒）。
# 留这段是为了让"松手解锁"（置信度掉下来→清锁）仍能生效，别设得太短。
MOTION_GATE_GRACE_MS = 2500
# 变化像素占比超过它才算有动作（0.004 ≈ 80x60 分析图里的 19 个像素）。
# 这个值是拿真实摄像头实测标定的：静止画面连拍 180 帧，加 5x5 模糊后
# 变化像素最大只有 10 个，19 个约是它的 2 倍，对应原图 640x480 里约 45x45 像素的变化。
# 取值刻意偏敏感：太钝会把手势误挡掉（功能坏了），太灵只是省不下多少算力。
...
TUNABLE: list = [
    ...
    {"key": "MOTION_THRESHOLD", "label": "动静门控灵敏度",
     "min": 0.0005, "max": 0.0200, "decimals": 4,
     "hint": "画面变化超过这个比例才算“有人在动”。调大更省但可能漏掉幅度小的手势，"
             "调小更灵敏但省得少。注意：门控总开关在 config.MOTION_GATE_ENABLED，默认关闭。"},
    {"key": "MOTION_GATE_GRACE_MS", "label": "门控宽限期",
     "min": 500, "max": 10000, "decimals": 0, "unit": "ms",
     "hint": "动作停止后仍继续推理这么久，保证“松手解锁”能生效。别设太短。"},
]
