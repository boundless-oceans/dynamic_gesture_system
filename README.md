...
展台多数时间没人在做手势，而一次推理要几百毫秒。门控用"帧间差异"判断画面里有没有动作，
静止时跳过推理，一有动作立刻恢复——**不增加延迟**（对比"拉长推理间隔"，那个会同时降低
时间覆盖率和响应速度，而且在越慢的机器上收益越小）。

**默认关闭**（`config.MOTION_GATE_ENABLED = False`），因为收益强依赖于现场光线与摄像头：

| 阶段 | 静止画面误报率 | 实测门控空闲占比 |
|---|---|---|
| 初始参数（160×120、无滤波、阈值 0.002） | 15.7% | 几乎为 0（噪声把门控一直撑开） |
| 实测标定后（80×60、5×5 模糊、阈值 0.004，即当前默认） | 约 3% | 15%~40% |

标定方法：对静止画面连拍 180~300 帧，统计"变化像素数"分布，把阈值取在噪声最大值之上。

**建议到现场后再决定是否打开**，打开前先跑 `tests/manual/smoke_inference.py` 确认识别正常，
并留意终端是否正常出现 `画面静止，暂停推理` 与 `检测到动作，恢复推理`。
灵敏度可用设置页「参数调节 → 动静门控灵敏度」现场微调；宽限期别调太短，否则"松手解锁"会失效。
